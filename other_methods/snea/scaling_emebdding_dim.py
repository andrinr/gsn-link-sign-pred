...
parser.add_argument('--total_minibatches', type=int, default=4000)
...
# ================================================================================= #
embedding_dims = np.logspace(4, 7, 4, base=2).astype(int)
times = []
for embedding_dim in embedding_dims:
    ...
    #             )
    test_adj_lists_pos = None
    test_adj_lists_neg = None
    num_nodes = 10000
    num_edges = num_nodes * 9
    print(f"Number of nodes: {num_nodes}, number of edges: {num_edges}")
    num_feats = embedding_dim
    print(f"Number of features: {num_feats}")
    args['num_input_features'] = num_feats
    signs = np.random.randint(0, 2, (num_edges,))
    num_edges_pos = np.sum(signs)
    num_edges_neg = num_edges - num_edges_pos
    feat_data = np.random.rand(num_nodes, num_feats)
    ...
    for i in range(num_nodes):
        adj_lists_pos.append(set())
        adj_lists_neg.append(set())
        adj_lists_pos[i].add(np.random.randint(0, num_nodes))
        adj_lists_neg[i].add(np.random.randint(0, num_nodes))

    for edge in edge_index:
        if signs[edge[0]] == 1:
            adj_lists_pos[edge[0]].add(edge[1])
        else:
            adj_lists_neg[edge[0]].add(edge[1])

    num_edges = [num_edges_pos, num_edges_neg]
    ...
    times.append((end - start) * total_batches / 10)
