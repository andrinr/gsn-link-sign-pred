...
def evalute(
    ...
    evauluation_mask : jnp.ndarray) -> Metrics:
    ...
    y_pred = logreg.predict(spring_vec_norm.at[evauluation_mask].get())

    auc = roc_auc_score(signs.at[evauluation_mask].get(), y_pred)
    f1_binary = f1_score(signs.at[evauluation_mask].get(), y_pred, average='binary')
    f1_micro = f1_score(signs.at[evauluation_mask].get(), y_pred, average='micro')
    f1_macro = f1_score(signs.at[evauluation_mask].get(), y_pred, average='macro')
